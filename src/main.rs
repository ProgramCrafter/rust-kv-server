...
impl Node for Tag {
    fn write(&self, stream: &mut BufWriter<Box<dyn Write>>) -> std::io::Result<()> {
        ...
        if self.tag_name == "smc" {
            ...
            let writer_cptr: *const Box<Cursor<Vec<u8>>> =
                writer_dptr as *const Box<Cursor<Vec<u8>>>;

            let writer_a: &Box<Cursor<Vec<u8>>> = unsafe { &(*writer_cptr) };

            let data: &Vec<u8> = writer_a.get_ref();
            let s: &str = std::str::from_utf8(data).unwrap();

            stream.write_all(
                s.replace("\0", self.children.len().to_string().as_str())
                    .as_bytes(),
            )?;
        } else {
            for child_node in &self.children {
...
fn boxed_page(head_nodes: Vec<Box<dyn Node>>, body_nodes: Vec<Box<dyn Node>>) -> Box<Tag> {
    let head = Box::new(Tag {
        tag_name: "head".to_string(),
        children: head_nodes,
        properties: HashMap::new(),
    });
    let body = Box::new(Tag {
...
fn process_style_template(
    prop_name: &str,
    prop_val: &str,
    node: &mut Box<Tag>,
    global_keys: &mut HashMap<String, String>,
) {
